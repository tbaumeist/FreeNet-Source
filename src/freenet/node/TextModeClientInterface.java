...
public class TextModeClientInterface implements Runnable {
	...
	public TextModeClientInterface(Node n, HighLevelSimpleClient c,
	...
	private void printHeader(OutputStream s) throws IOException {
		...
		sb.append("---------------------------------------\r\n");
		sb.append("Freenet 0.7.5 Build #").append(Version.buildNumber())
		...
		sb
				.append("PUT:<text> - Put a single line of text to a CHK and return the key.\r\n");
		sb
				.append("PUTHTL:htl:<text> - Put a single line of text to a CHK and return the key. Specifying the htl to use.\r\n");
		sb
		...
		sb
				.append("GETCHKFILE:<filename> - Get the key that would be returned if we inserted the file.\r\n");
		sb
				.append("PUTDIR:<path>[#<defaultfile>] - Put the entire directory from disk.\r\n");
		sb
				.append("GETCHKDIR:<path>[#<defaultfile>] - Get the key that would be returned if we'd put the entire directory from disk.\r\n");
		sb.append("MAKESSK - Create an SSK keypair.\r\n");
		sb
				.append("PUTSSK:<insert uri>;<url to redirect to> - Insert an SSK redirect to a file already inserted.\r\n");
		sb
				.append("PUTSSKDIR:<insert uri>#<path>[#<defaultfile>] - Insert an entire directory to an SSK.\r\n");
		sb
				.append("PLUGLOAD: - Load plugin. (use \"PLUGLOAD:?\" for more info)\r\n");
		...
		sb
				.append("PLUGKILL:<pluginID> - Unload the plugin with the given ID (see PLUGLIST).\r\n");
		...
		sb
				.append("CONNECT:<filename|URL> - see ADDPEER:<filename|URL> below\r\n");
		sb
				.append("CONNECT:\\r\\n<noderef> - see ADDPEER:\\r\\n<noderef> below\r\n");
		sb
				.append("DISCONNECT:<ip:port|name> - see REMOVEPEER:<ip:port|name|identity> below\r\n");
		sb
				.append("ADDPEER:<filename|URL> - add a peer from its ref in a file/url.\r\n");
		sb
				.append("ADDPEER:\\r\\n<noderef including an End on a line by itself> - add a peer by entering a noderef directly.\r\n");
		sb
				.append("DISABLEPEER:<ip:port|name|identity> - disable a peer by providing its ip+port, name, or identity\r\n");
		sb
				.append("ENABLEPEER:<ip:port|name|identity> - enable a peer by providing its ip+port, name, or identity\r\n");
		sb
				.append("SETPEERLISTENONLY:<ip:port|name|identity> - set ListenOnly on a peer by providing its ip+port, name, or identity\r\n");
		sb
				.append("UNSETPEERLISTENONLY:<ip:port|name|identity> - unset ListenOnly on a peer by providing its ip+port, name, or identity\r\n");
		sb
				.append("HAVEPEER:<ip:port|name|identity> - report true/false on having a peer by providing its ip+port, name, or identity\r\n");
		sb
				.append("REMOVEPEER:<ip:port|name|identity> - remove a peer by providing its ip+port, name, or identity\r\n");
		sb
				.append("PEER:<ip:port|name|identity> - report the noderef of a peer (without metadata) by providing its ip+port, name, or identity\r\n");
		sb
				.append("PEERWMD:<ip:port|name|identity> - report the noderef of a peer (with metadata) by providing its ip+port, name, or identity\r\n");
		sb
				.append("PEERS - report tab delimited list of peers with name, ip+port, identity, location, status and idle time in seconds\r\n");
		...
		sb
				.append("STATUS - display some status information on the node including its reference and connections.\r\n");
		sb
				.append("MEMSTAT - display some memory usage related informations.\r\n");
		sb.append("SHUTDOWN - exit the program\r\n");
		sb
				.append("ANNOUNCE[:<location>] - announce to the specified location\r\n");
		// custom TMCI command
		sb
				.append("PEERFILE:<[peer type]> - dump list of peers to file. peer type=ALL, dumps all peers. peer type=CONNECTED, dumps only connected peers\r\n");
		...
		sb
				.append("TRACEBACKATTACK: Message UID - Finds all nodes that routed the message with the given UID.\r\n");
		sb
				.append("PRINTMESSAGEUIDS Prints all of the compted message UIDS stored.\r\n");
		sb
				.append("ATTACKAGENT: Attack Cload IPs , seperated - Turns this node into an attack agent that uses the attack cload at given ip to perform the attack.\r\n");
		sb
				.append("ATTACKAGENTINSERTFILTER: TRUE | FALSE - Turns the insert attack agent on/off.\r\n");
		sb
				.append("ATTACKAGENTREQUESTFILTER: TRUE | FALSE - Turns the update attack agent on/off.\r\n");
		...
		if (n.testnetEnabled) {
			sb
					.append("WARNING: TESTNET MODE ENABLED. YOU HAVE NO ANONYMITY.\r\n");
		}
	 ...
	 * 
	 * @throws IOException
	 *             If we could not write the data to stdout.
	 */
	private boolean processLine(BufferedReader reader, final OutputStream out)
		...
		if (uline.startsWith("GET:")) {
			...
			try {
				...
				if (evil) {
					System.err
							.println("Data may contain escape codes which could cause the terminal to run arbitrary commands! Save it to a file if you must with GETFILE:");
					outsb
							.append("Data may contain escape codes which could cause the terminal to run arbitrary commands! Save it to a file if you must with GETFILE:");
					outsb.append("\r\n");
					out.write(outsb.toString().getBytes());
					out.flush();
					return false;
			...
			try {
				...
				ClientGetter get = new ClientGetter(fw, uri, context,
						...
						(RequestClient) client, null, null);
			...
			try {
				...
				if (fnam.length() == 0) {
					...
					String ext = DefaultMIMETypes
							.getExtension(cm.getMIMEType());
				...
				outsb.append("Download rate: ").append(rate).append(
						" bytes / second");
			} catch (FetchException e) {
			...
			try {
				...
				ContentFilter.filter(inputStream, outputStream, "text/html",
			...
			n.getNodeUpdater().blow(
					"caught an  IOException : (Incompetent Operator) :p", true);
			outsb.append("\r\n");
			out.write(outsb.toString().getBytes());
			out.flush();
			return false;
			...
			// note can't have : in content, but this is just for us so it should be ok
			line = parsed[1];
			...
			this.n.disableProbabilisticHTLs = true;
			InsertBlock block = new InsertBlock(new ArrayBucket(data), null,
					FreenetURI.EMPTY_CHK_URI);
			...
			this.n.disableProbabilisticHTLs = b;
			outsb.append("URI: ").append(uri);
			// custom
			ClientCHK chk = new ClientCHK(uri);
			double targetLoc = chk.getNodeCHK().toNormalizedDouble();
			...
			if (defaultFile == null) {
				String[] defaultFiles = new String[] { "index.html",
			...
			FreenetURI uri;
			try {
				uri = client.insertManifest(insertURI, bucketsByName,
						defaultFile);
			...
			try {
				...
				FileBucket fb = new FileBucket(f, true, false, false, false,
						false);
				InsertBlock block = new InsertBlock(fb, new ClientMetadata(
						mimeType), FreenetURI.EMPTY_CHK_URI);
				...
				long sz = f.length();
				double rate = 1000.0 * sz / (endTime - startTime);
				outsb.append("Upload rate: ").append(rate).append(
						" bytes / second\r\n");
			...
			outsb.append("Insert URI: ").append(
					key.getInsertURI().toString(false, false)).append("\r\n");
			outsb.append("Request URI: ").append(
					key.getURI().toString(false, false)).append("\r\n");
			...
			outsb
					...
					.append(key.getURI().toString(false, false)).append(
							"testsite/");
			...
			try {
				...
				outsb.append("Successfully inserted to fetch URI: ").append(
						result);
			} catch (InsertException e) {
				outsb.append("Finished insert but: ").append(e.getMessage());
			...
			if (Version.buildNumber() < Version.getHighestSeenBuild()) {
				outsb.append("The latest version is : ").append(
						Version.getHighestSeenBuild());
			}
			...
			if (key.length() > 0) {
				...
				if (f.isFile()) {
					outsb
							.append("Given string seems to be a file, loading...\r\n");
					in = new BufferedReader(new FileReader(f));
				} else {
					outsb
							.append("Given string seems to be an URL, loading...\r\n");
					URL url = new URL(key);
			...
			String nodeIdentifier = (line.substring("DISABLEPEER:".length()))
					.trim();
			if (!havePeer(nodeIdentifier)) {
				out
						.write(("no peer for " + nodeIdentifier + "\r\n")
								.getBytes());
				out.flush();
				return false;
			}
			...
			String nodeIdentifier = (line.substring("ENABLEPEER:".length()))
					.trim();
			if (!havePeer(nodeIdentifier)) {
				out
						.write(("no peer for " + nodeIdentifier + "\r\n")
			...
			String nodeIdentifier = (line.substring("SETPEERLISTENONLY:"
					.length())).trim();
			if (!havePeer(nodeIdentifier)) {
				out
						.write(("no peer for " + nodeIdentifier + "\r\n")
			...
			if (pn == null) {
				out.write(("n.getPeerNode() failed to get peer details for "
						+ nodeIdentifier + "\r\n\r\n").getBytes());
				out.flush();
				return false;
			}
			if (!(pn instanceof DarknetPeerNode)) {
				out
						.write(("Error: " + nodeIdentifier + " identifies a non-darknet peer and this command is only available for darknet peers\r\n\r\n")
								.getBytes());
				out.flush();
				return false;
			}
			...
			outsb.append("set ListenOnly suceeded for ").append(nodeIdentifier)
					.append("\r\n");
		} else if (uline.startsWith("UNSETPEERLISTENONLY:")) {
			String nodeIdentifier = (line.substring("UNSETPEERLISTENONLY:"
					.length())).trim();
			if (!havePeer(nodeIdentifier)) {
				out
						.write(("no peer for " + nodeIdentifier + "\r\n")
			...
			outsb.append("unset ListenOnly suceeded for ").append(
					nodeIdentifier).append("\r\n");
		} else if (uline.startsWith("HAVEPEER:")) {
			String nodeIdentifier = (line.substring("HAVEPEER:".length()))
					.trim();
			if (havePeer(nodeIdentifier)) {
			...
			String nodeIdentifier = (line.substring("PEERWMD:".length()))
					.trim();
			if (!havePeer(nodeIdentifier)) {
				out
						.write(("no peer for " + nodeIdentifier + "\r\n")
		...
		// custom TMCI command
			...
			outsb.append(n.writeChkDatastoreFileB()); // writes store file
			// iterating over db
			// with cursor
			outsb.append("\r\nSTOREFILEB done.\r\n");
			...
			ProbeCallback cb = new ProbeCallback() {
				public void onCompleted(String reason, double target,
					...
					String msg = "Completed probe request: " + target + " -> "
							+ best + "\r\nNearest actually hit " + nearest
							+ ", " + counter + " nodes (" + uniqueCounter
							+ " unique, " + linearCounter + " hops), id " + id
							+ "\r\n";
					try {
				...
				public void onTrace(long uid, double target, double nearest,
						double best, short htl, short counter, double location,
						long nodeUID, double[] peerLocs, long[] peerUIDs,
						double[] locsNotVisited, short forkCount,
						short linearCounter, String reason, long prevUID) {
					String msg = "Probe trace: UID=" + uid + " target="
							+ target + " nearest=" + nearest + " best=" + best
							+ " htl=" + htl + " counter=" + counter
							+ " linear=" + linearCounter + " location="
							+ location + "node UID=" + nodeUID + " prev UID="
							+ prevUID + " peer locs="
							+ Arrays.toString(peerLocs) + " peer UIDs="
							+ Arrays.toString(peerUIDs)
							+ " locs not visited = "
							+ Arrays.toString(locsNotVisited) + " forks: "
							+ forkCount + " reason=" + reason + '\n';
					try {
						out.write(msg.getBytes());
						out.flush();
					} catch (IOException e) {
			...
			ProbeCallback cb = new ProbeCallback() {
				...
				public void onCompleted(String reason, double target,
					...
					for (Double sourceLoc : topology.keySet()) {
						for (Double peerLoc : topology.get(sourceLoc).keySet()) {
							...
							double peerRounded = Double.valueOf(threeDForm
									.format(peerLoc.doubleValue() + 1));

							msg += "\"" + sourceLoc + "\" -> \""
									+ Math.abs(peerRounded) + "\"\n";
						}
					}
					msg += "}\n";
					try {
				...
				public void onTrace(long uid, double target, double nearest,
					...
					if (!topology.containsKey(dLocation))
						topology
								.put(dLocation, new Hashtable<Double, Object>());
					...
					for (double peerLoc : peerLocs) {
						...
						if (!peers.containsKey(dPeerLoc))
							peers.put(dPeerLoc, new Object());// should use a
						// list here but
						// w/e
					}
				}
			...
			if (uline.startsWith("PLUGLOAD:O:")) {
				...
				outsb
						.append("  PLUGLOAD:O: pluginName         - Load official plugin from freenetproject.org\r\n");
				outsb
						.append("  PLUGLOAD:F: file://<filename>  - Load plugin from file\r\n");
				outsb
						.append("  PLUGLOAD:U: http://...         - Load plugin from online file\r\n");
				outsb
						.append("  PLUGLOAD:K: freenet key        - Load plugin from freenet uri\r\n");
			}
		} else if (uline.startsWith("PLUGLIST")) {
			outsb.append(n.pluginManager.dumpPlugins());
		} else if (uline.startsWith("PLUGKILL:")) {
			n.pluginManager.killPlugin(line.substring("PLUGKILL:".length())
					.trim(), 60 * 1000, false);
	...
	/**
	 * Create a map of String -> Bucket for every file in a directory and its
	 * subdirs.
	 */
		...
		for (int i = 0; i < filelist.length; i++) {
			...
			if (filelist[i].canRead() && filelist[i].exists()) {
				if (filelist[i].isFile()) {
					...
					HashMap<String, Object> subdir = makeBucketsByName(directory
							+ filelist[i].getName());
	...
	/**
	 * @return A block of text, input from stdin, ending with a . on a line by
	 *         itself. Does some mangling for a fieldset if isFieldSet.
	 */
			...
			if (isFieldSet) {
				...
				if (line.equals("End")) {
					...
					if (line.endsWith("End")
							&& Character.isWhitespace(line.charAt(line.length()
									- ("End".length() + 1)))) {
						line = "End";
						breakflag = true;
					} else {
	...
	/**
	 * Disable connecting to a peer given its ip and port, name or identity, as
	 * a String Report peer success as boolean
	 */
	...
	/**
	 * Enable connecting to a peer given its ip and port, name or identity, as a
	 * String Report peer success as boolean
	 */
	...
	/**
	 * Check for a peer of the node given its ip and port, name or identity, as
	 * a String Report peer existence as boolean
	 */
	...
	/**
	 * Remove a peer from the node given its ip and port, name or identity, as a
	 * String Report peer removal successfulness as boolean
	 */
	private boolean removePeer(String nodeIdentifier) {
		...
		for (int i = 0; i < pn.length; i++) {
			...
			// String name = pn[i].myName;
			String identity = pn[i].getIdentityString();
			System.out.println("Comparing " + nodeIdentifier + " to "
					+ identity + ", " + nodeIpAndPort);
			// if(identity.equals(nodeIdentifier) ||
			// nodeIpAndPort.equals(nodeIdentifier) ||
			// name.equals(nodeIdentifier))
			if (identity.equals(nodeIdentifier)
